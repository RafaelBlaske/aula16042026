...
- RF09 — Relatórios Gerenciais O gerente deve poder emitir relatórios como: inadimplência, alunos ativos, histórico de acessos e ocupação das aulas
- RF10 — Notificações O sistema deve enviar notificações ao aluno sobre: vencimento de mensalidade, confirmação de agendamento e liberação de nova avaliação física
